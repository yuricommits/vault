...
import { LayoutGrid, Hash, Settings, User } from "lucide-react";
...
function NavItem({
    ...
    return (
        <Link
            href={href}
            className={`flex items-center gap-[10px] px-[10px] py-[8px] text-[12px] font-mono rounded-sm transition-all duration-[0.18s] border-l-[2px] ${
                isActive
        ...
        >
            <Icon
                size={14}
                className={isActive ? "text-text-1" : "text-text-4"}
            />
            <span>{label}</span>
        </Link>
    );
